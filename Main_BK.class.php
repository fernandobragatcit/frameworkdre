...
//			self::registraLogs(); //- com problemas
    ...
    private function getPopUp() {
        $popUp = ControlDb::getCol("SELECT exibir_popup FROM fwk_config_sistema WHERE id_config=1");
        if ($popUp['exibir_popup'] == "S") {
    ...
    private function setLimitMemory() {
        ...
        if (!empty($memory_limit_sistema["memory_limit"])) {
            ini_set("memory_limit", $memory_limit_sistema["memory_limit"]);
        }
    }

    private function getLarguraPortal() {
        $largura_portal = ControlDb::getCol("SELECT largura_portal FROM fwk_config_sistema WHERE id_config=1");
        $largura_portal = $largura_portal["largura_portal"];
