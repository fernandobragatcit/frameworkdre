                    ...
                    {else}
                        <div style="width: 100%;">
                            <span style="float:left; width:100%; margin-top:5px;">
                                {if $ARR_LINHAS[per].tipo == 'label'}
                                    <label class="{$ARR_LINHAS[per].class}" id="{$ARR_LINHAS[per].id}"  >{$ARR_LINHAS[per].label}{$ARR_LINHAS[per].obrigatorio}{if $ARR_LINHAS[per].label}:{else}{/if}</label>
                                {else}
                                    <label>{$ARR_LINHAS[per].label}{$ARR_LINHAS[per].obrigatorio}{if $ARR_LINHAS[per].label}:{else}{/if}</label>
                                {/if}
                                ...
                                {elseif $ARR_LINHAS[per].tipo == 'select'}
                                    <select {if $ARR_LINHAS[per].class != ""} class="{$ARR_LINHAS[per].class}{if $ARR_LINHAS[per].contadorClass=="1"}{$id+1}{/if}"{else}style="width: 100%;"{/if} title="{$ARR_LINHAS[per].label}" name="{$IDCOMPONENTE}[{if $ARR_LINHAS[per].contadorId=="1"}{$id+1}{else}{$id}{/if}][{$ARR_LINHAS[per].id}]"
                                                                              id="{$IDCOMPONENTE}[{if $ARR_LINHAS[per].contadorId=="1"}{$id+1}{else}{$id}{/if}][{$ARR_LINHAS[per].id}]" {if $ARR_LINHAS[per].contadorOnChange==1}{$ARR_LINHAS[per].onchange|replace:"[vlChange]":$id+1}{else}{$ARR_LINHAS[per].onchange}{/if} >
...
<div class="left adicionarBloco" style="margin-top:10px;">
    {if $SCRIPT_ADD_BLOCO}
        <span align="left"><strong><a href="javascript:{$SCRIPT_ADD_BLOCO}('{$IDCOMPONENTE}');" class="darkBrown adicionaNovoBloco">[+] Adicionar nova</a></strong></span>
                {else}
        <span align="left"><strong><a href="javascript:addBlocoCampo('{$IDCOMPONENTE}');" class="darkBrown adicionaNovoBloco">[+] Adicionar nova</a></strong></span>
                {/if}
                    ...
                    {else}
                        <div style="width: 100%;">
                            <span style="float:left; width:100%; margin-top:5px;">
                                ...
                                {elseif $ARR_LINHAS[per].tipo == 'select'}
                                    <select {if $ARR_LINHAS[per].class != ""}class="{$ARR_LINHAS[per].class}{if $ARR_LINHAS[per].contadorClass=="1"}{$id+1}{/if}"{else}style="width: 100%;"{/if} title="{$ARR_LINHAS[per].label}" name="{$IDCOMPONENTE}[{if $ARR_LINHAS[per].contadorId=="1"}{$number+1}{else}{$number}{/if}][{$ARR_LINHAS[per].id}]"
                                                                             id="{$IDCOMPONENTE}[{if $ARR_LINHAS[per].contadorId=="1"}{$number+1}{else}{$number}{/if}][{$ARR_LINHAS[per].id}]" {if $ARR_LINHAS[per].contadorOnChange==1}{$ARR_LINHAS[per].onchange|replace:"[vlChange]":$number+1}{else}{$ARR_LINHAS[per].onchange}{/if}>
                                        <option value="">Selecione...</option>
                                        {section name=op loop=$ARR_LINHAS[per].option}
                                            <option {if $DADOS[da][$val] == $ARR_LINHAS[per].option[op].value} selected {/if} value="{$ARR_LINHAS[per].option[op].value}">{$ARR_LINHAS[per].option[op].label}</option>
                                        {/section}
