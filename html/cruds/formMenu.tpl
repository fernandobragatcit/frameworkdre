<p>
	Os campos marcados com <span class="campoObrigatorio">*</span> são de preenchimento obrigatório
</p>

<div class="formTelaFull">
	<label>{$nome_menu_label} :
	    <span class="small error" id="nome_menu_erro">{$nome_menu_error}</span>
	</label> {$nome_menu_campo}
</div>

<div class="formTelaFull">
	<label>{$link_menu_label} :
		<span class="small error" id="link_menu_erro">{$link_menu_error}</span>
	</label> {$link_menu_campo}
</div>

<div class="formTelaFull">
	<label>{$id_portal_label} :
		<span class="small error" id="id_portal_erro">{$id_portal_error}</span>
	</label> {$id_portal_campo}
</div>

<div class="formTelaFull">
	<label>{$funcionalidade_label} :
	    <span class="small error" id="funcionalidade_erro">{$funcionalidade_error}</span>
	</label> {$funcionalidade_campo}
</div>

<div class="formTelaFull">
	<label>{$ordem_menu_label} :
		<span class="small error" id="ordem_menu_erro">{$ordem_menu_error}</span>
	</label> {$ordem_menu_campo}
</div>

<div class="formTelaFull">
	<div class="alinhaBtns">{$Cadastrar_campo} {$Cancelar_campo}</div>
</div>