...
<label>{$nome_imagem_label} :
    <span class="small" id="nome_imagem_erro" style="color:red">{$nome_imagem_error}</span>
</label> {$nome_imagem_campo}

<p style="padding-top:10px;">
	<strong>Categoria não é obrigatória, mas serve para agrupar em caso de banners rotativos</strong>
</p>
<label>{$id_categoria_banner_label} :
	<span class="small" id="id_categoria_banner_erro">{$id_categoria_banner_error}</span>
</label> {$id_categoria_banner_campo}

<label>{$title_banner_label} :
	<span class="small" id="title_banner_erro">{$title_banner_error}</span>
</label> {$title_banner_campo}

<label>{$link_banner_label} :
	<span class="small" id="link_banner_erro">{$link_banner_error}</span>
</label> {$link_banner_campo}
