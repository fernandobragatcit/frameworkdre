	...
	*/
	...
	function Securimage() {
		...
		$this->multi_text_color = array (
			...
			0x20,
			0xCC
		), new Securimage_Color(0x0, 0x30, 0xEE), new Securimage_color(0x0, 0x40, 0xCC), new Securimage_Color(0x0, 0x50, 0xEE), new Securimage_Color(0x0, 0x60, 0xCC));
...
die();*/
		if (isset ($_SESSION["securimage_code_value"]) && trim($_SESSION["securimage_code_value"]) != '') {
			...
			if ($this->use_sqlite_db == true && function_exists('sqlite_open')) { // no code in session - may mean user has cookies turned off
				$this->openDatabase();
